...
public class LuckyWheelManager : MonoBehaviour, IMapmakerModule
{
    ...
    private void Start()
    {
        ...
        if (wheel && !m_CurView && m_Enable)
        {
            m_CurView = Instantiate(m_WheelViewPrefab, MapManager.Instance.UICanvas.transform).GetComponent<LuckyWheelView>();
            m_CurView.SetWheel(wheel);
        }
    }
